...
if filtered == 1:
    files = sorted(glob(f"{diri}filtered/{file_name}.{mode_file_name}.{year_start:04d}-{year_end:04d}.nc"))
    
    if len(files) == 0:
        sys.exit(f"No .nc file for {diri}filtered/{file_name}.{mode_file_name}.{year_start:04d}-{year_end:04d}")
    ...
    fileo = f"{diro}composite.anom.{file_name}.{mode_file_name}.{year_start:04d}-{year_end:04d}.nc"
