...
script_filename = "calc_mean_precipitation_in_deep_tropics_associated_with_mode_with_threshold.py"
...
env_vars["DIRO"] = "/scratch/k10/mr4682/data/GPM/3hr_mean_precipitation_associated_with_mode/with_threshold/"
...
for year in years:
    ...
    for month in np.arange(month_start, month_end + 1, 1, dtype=int):
        ...
        files = sorted(glob(f"{env_vars['DIRO']}precipitation.in.deep.tropics.a.w.mode.w.t.{env_vars['THRESHOLD']:.2f}.{year:04d}{month:02d}.nc"))
        
        if len(files) != 0:
            continue
        
        name = f"{script_filename[:-3]}__{env_vars['THRESHOLD']:.2f}_{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}"
