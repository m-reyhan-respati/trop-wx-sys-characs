...
fileo = f"{diro}mean.torrential.contribution.{file_name}.w.t.{threshold:.2f}.{mode_dict[mode]}.circ.mask.{radius:.1f}km.{lat_min_string}-{lat_max_string}.nc"

def _preprocess(ds):
    return ds.sel(lat=slice(lat_min + 0.01, lat_max - 0.01))

files = sorted(glob(f"{diri}torrential.contribution.{file_name}.w.t.{threshold:.2f}.*.circ.mask.{radius:.1f}km.nc"))

ds = xr.open_mfdataset(files, preprocess=_preprocess)

x = ds[f"torrential_contribution_{var_name}_{modes[mode]}"]

x_all = x.mean(dim="time", skipna=True).compute()

dso = xr.Dataset()
dso[f"mean_torrential_contribution_{var_name}_{modes[mode]}_ALL"] = x_all

seasons = ["DJF", "MAM", "JJA", "SON"]

for season in seasons:
    x_season = x[x["time"].dt.season.values == season, :, :].mean(dim="time", skipna=True).compute()
    
    dso[f"mean_torrential_contribution_{var_name}_{modes[mode]}_{season}"] = x_season
