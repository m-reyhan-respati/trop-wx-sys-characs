import numpy as np
from glob import glob

from config import *

import submit_job

script_dir = f"{ROOT_DIR}/scripts/"
script_filename = "calc_mse.py"

year_start = 1984
year_end = 2016

years = np.arange(year_start, year_end + 1, 1, dtype=int)

env_vars = {}
env_vars["DIRI"] = f"{SCRATCH_ERA5_DIR}/"
env_vars["DIRO"] = f"{SCRATCH_ERA5_DIR}/mse/tmp/"

pbs_dir = f"{ROOT_DIR}/pbs_scripts/"
ncpus = 1
mem = 10
jobfsmem = 1
queue = "normal"
project = "if69"
walltime = "00:15:00"
storage = "gdata/xp65+scratch/k10"
command = f"""cd {ROOT_DIR}

source env.sh

python3 {script_dir}{script_filename}"""

levels = [100, 125, 150, 175, 200, 225, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 775, 800, 825, 850, 875, 900, 925, 950, 975, 1000]

for level in levels:
    jobs_id = []

    env_vars["LEVEL"] = level

    for year in years:
        env_vars["YEAR"] = year

        files = sorted(glob(f"{env_vars['DIRO']}mse.{env_vars['LEVEL']}.{env_vars['YEAR']}.nc"))

        if len(files) != 0:
            continue

        name = f"{script_filename[:-3]}__{env_vars['LEVEL']}_{env_vars['YEAR']}"
        pbs_filename = f"{name}.sh"

        pbs_script = submit_job.submit.create_pbs_script(pbs_dir, pbs_filename, ncpus, mem, jobfsmem, queue, project, walltime, storage, name, command)

        submit_job.submit.submit_job(env_vars, pbs_script, jobs_id)

    if len(jobs_id) != 0:
        submit_job.check.check_job_status(jobs_id, time_delay=300)
