...
if filtered == 1:
    files = sorted(glob(f"{diri}filtered/stacks/stack.{file_name}.{mode_file_name}.*.nc"))
    
    if len(files) == 0:
        sys.exit(f"No .nc file for {diri}filtered/stacks/stack.{file_name}.{mode_file_name}")
    
    fileo = f"{diro}composite.{file_name}.{mode_file_name}.lag.{lag:.0f}.nc"
elif filtered == 0:
    files = sorted(glob(f"{diri}stacks/stack.anom.{file_name}.{mode_file_name}.*.nc"))
    
    if len(files) == 0:
        sys.exit(f"No .nc file for {diri}stacks/stack.anom.{file_name}.{mode_file_name}")
    
    fileo = f"{diro}composite.anom.{file_name}.{mode_file_name}.lag.{lag:.0f}.nc"
