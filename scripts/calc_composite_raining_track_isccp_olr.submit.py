...
script_filename = "calc_composite_raining_track_isccp_olr.py"
...
for level in levels:
    ...
    for mode in modes:
        ...
        if env_vars["FILTERED"] == 0:
            files = sorted(glob(f"{env_vars['DIRO']}composite*raining.anom.{env_vars['FILE_NAME']}.{mode_dict[mode]}.nc"))
            name = f"{script_filename[:-3]}__anom_{env_vars['FILE_NAME']}_{mode_dict[mode]}"
        elif env_vars["FILTERED"] == 1:
            files = sorted(glob(f"{env_vars['DIRO']}composite*raining.{env_vars['FILE_NAME']}.{mode_dict[mode]}.nc"))
            name = f"{script_filename[:-3]}__{env_vars['FILE_NAME']}_{mode_dict[mode]}"
