...
script_filename = "calc_freq_duration_3hr.py"
...
env_vars = {}
env_vars["DIRO"] = "/scratch/k10/mr4682/data/GPM/3hr_duration/freq_duration_3hr/"
...
for i in range(0, len(lat_min_list)):
    ...
    files = sorted(glob(f"{env_vars['DIRO']}freq.duration.3hr.{lat_min_string}_{lat_max_string}.nc"))
    
    if len(files) != 0:
        continue
    
    name = f"{script_filename[:-3]}__{lat_min_string}_{lat_max_string}"
