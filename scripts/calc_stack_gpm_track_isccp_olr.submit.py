...
script_filename = "calc_stack_gpm_track_isccp_olr.py"
    ...
    for month in np.arange(month_start, month_end + 1, 1, dtype=int):
        ...
        for mode in modes[9:10]:
            ...
            if env_vars["FILTERED"] == 0:
                files = sorted(glob(f"{env_vars['DIRO']}stack.anom.{env_vars['FILE_NAME']}.{mode_dict[mode]}.{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}.nc"))
                name = f"{script_filename[:-3]}__anom_{env_vars['FILE_NAME']}_{mode_dict[mode]}_{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}"
            elif env_vars["FILTERED"] == 1:
                files = sorted(glob(f"{env_vars['DIRO']}stack.{env_vars['FILE_NAME']}.{mode_dict[mode]}.{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}.nc"))
