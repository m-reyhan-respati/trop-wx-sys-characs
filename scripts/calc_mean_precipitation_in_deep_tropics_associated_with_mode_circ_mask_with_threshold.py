...
mask_moisture_mode = calc_circ_mask(track1, time, lat, lon, radius, grid_res)[:, (lat.values >= -20.05) & (lat.values <= 20.05), :]
mask_mixed_system = calc_circ_mask(track2, time, lat, lon, radius, grid_res)[:, (lat.values >= -20.05) & (lat.values <= 20.05), :]
mask_ig_wave = calc_circ_mask(track3, time, lat, lon, radius, grid_res)[:, (lat.values >= -20.05) & (lat.values <= 20.05), :]
mask_tc = calc_circ_mask(track4, time, lat, lon, radius, grid_res)[:, (lat.values >= -20.05) & (lat.values <= 20.05), :]
mask_anom = calc_circ_mask(track5, time, lat, lon, radius, grid_res)[:, (lat.values >= -20.05) & (lat.values <= 20.05), :]
...
fileo = f"{diro}precipitation.in.deep.tropics.a.w.mode.w.t.{threshold:.2f}.{year:04d}{month:02d}.circ.mask.{radius:.1f}km.nc"
