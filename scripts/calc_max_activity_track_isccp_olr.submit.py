...
script_filename = "calc_max_activity_track_isccp_olr.py"

env_vars = {}
env_vars["DIRI"] = f"{SCRATCH_ISCCP_DIR}/txuptp/filtered/"
...
for mode in modes:
    ...
    files = sorted(glob(f"{env_vars['DIRO']}max.activity.track.txuptp.{mode_dict[mode]}.{env_vars['YEAR_START']}-{env_vars['YEAR_END']}.csv"))
