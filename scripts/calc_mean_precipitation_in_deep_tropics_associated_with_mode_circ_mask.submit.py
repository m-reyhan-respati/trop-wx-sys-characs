...
script_filename = "calc_mean_precipitation_in_deep_tropics_associated_with_mode_circ_mask.py"
...
env_vars = {}
env_vars["RADIUS"] = 500.0
env_vars["DIRO"] = "/scratch/k10/mr4682/data/GPM/3hr_mean_precipitation_associated_with_mode/circ_mask/"
...
for year in years:
    ...
    for month in np.arange(month_start, month_end + 1, 1, dtype=int):
        ...
        files = sorted(glob(f"{env_vars['DIRO']}precipitation.in.deep.tropics.a.w.mode.{year:04d}{month:02d}.circ.mask.{env_vars['RADIUS']:.1f}km.nc"))
        
        if len(files) != 0:
            continue
        
        name = f"{script_filename[:-3]}__{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}_{env_vars['RADIUS']:.1f}km"
