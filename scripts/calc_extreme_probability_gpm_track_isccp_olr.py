...
files = sorted(glob(f"{diri}extreme_mask/extreme.mask.{file_name}.{mode_file_name}.*.nc"))

if len(files) == 0:
    sys.exit(f"No .nc file for {diri}extreme_mask/extreme.mask.{file_name}.{mode_file_name}")

fileo = f"{diro}extreme.probability.{file_name}.{mode_file_name}.lag.{lag:.0f}.nc"
...
for i in range(0, len(files)):
    x_i = xr.open_dataset(files[i])[f"extreme_mask_{var_name}_{mode_var_name}"].sel(lag=slice(lag, lag))
...
for season in seasons:
    for domain in domains:
        probability, pvalue = track_cell.stats.calc_extreme_probability_gpm(x, max_activity, season, domain, H_0=0.05)
