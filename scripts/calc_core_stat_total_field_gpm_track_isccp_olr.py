...
margin = float(os.environ.get("MARGIN"))
...
fileo = f"{diro}core.{statistic}.{file_name}.{mode_file_name}.lag.{lag:.0f}.margin.{margin:.1f}deg.nc"
...
core_stat = track_cell.stats.calc_core_stat_gpm(x, max_activity, margin, statistic)

print(core_stat)

dso[f"core_{statistic}_{var_name}_{mode_var_name}"] = core_stat
