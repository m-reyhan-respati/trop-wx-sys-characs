...
fileo = f"{diro}mean.extreme.contribution.{percentile:d}p.{file_name}.{mode_dict[mode]}.{lat_min_string}-{lat_max_string}.nc"

def _preprocess(ds):
    return ds.sel(lat=slice(lat_min + 0.01, lat_max - 0.01))

files = sorted(glob(f"{diri}extreme.contribution.{percentile:d}p.{file_name}.*.nc"))
