...
script_filename = "calc_mean_torrential_contribution_gpm_track_isccp_olr_circ_mask.py"
...
env_vars["THRESHOLD"] = 0.0
env_vars["RADIUS"] = 1000.0
env_vars["DIRI"] = f"{SCRATCH_GPM_DIR}/3hr_mean_{env_vars['FILE_NAME']}/torrential_contribution/circ_mask/"
env_vars["DIRO"] = f"{SCRATCH_GPM_DIR}/3hr_mean_{env_vars['FILE_NAME']}/mean_torrential_contribution/circ_mask/"
...
for mode in modes[0:3]:
    ...
    for i in range(0, len(lat_min_list)):
        ...
        files = sorted(glob(f"{env_vars['DIRO']}mean.torrential.contribution.{env_vars['FILE_NAME']}.w.t.{env_vars['THRESHOLD']:.2f}.{mode_dict[mode]}.circ.mask.{env_vars['RADIUS']:.1f}km.{lat_min_string}-{lat_max_string}.nc"))
        name = f"{script_filename[:-3]}__{env_vars['FILE_NAME']}_{env_vars['THRESHOLD']:.2f}_{mode_dict[mode]}_circ_mask_{env_vars['RADIUS']:.1f}km_{lat_min_string}-{lat_max_string}"
