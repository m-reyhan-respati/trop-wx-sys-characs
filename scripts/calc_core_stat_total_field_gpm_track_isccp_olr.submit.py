...
script_filename = "calc_core_stat_total_field_gpm_track_isccp_olr.py"
...
env_vars["STATISTIC"] = "mean"
env_vars["MARGIN"] = 2.0
env_vars["DIRI"] = f"{SCRATCH_GPM_DIR}/3hr_mean_{env_vars['FILE_NAME']}/"
env_vars["DIRO"] = f"{env_vars['DIRI']}core_stats/"
...
for mode in modes[0:3]:
    ...
    for lag in lags:
        ...
        files = sorted(glob(f"{env_vars['DIRO']}core.{env_vars['STATISTIC']}.{env_vars['FILE_NAME']}.{mode_dict[mode]}.lag.{env_vars['LAG']:.0f}.margin.{env_vars['MARGIN']:.1f}deg.nc"))
        name = f"{script_filename[:-3]}__{env_vars['STATISTIC']}_{env_vars['FILE_NAME']}_{mode_dict[mode]}_lag_{env_vars['LAG']:.0f}_margin_{env_vars['MARGIN']}deg"
