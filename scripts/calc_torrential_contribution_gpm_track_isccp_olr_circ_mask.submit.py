...
script_filename = "calc_torrential_contribution_gpm_track_isccp_olr_circ_mask.py"
...
env_vars["DIRO"] = f"{env_vars['DIRI']}torrential_contribution/"
...
for year in years:
    ...
    for month in np.arange(month_start, month_end + 1, 1, dtype=int):
        ...
        files = sorted(glob(f"{env_vars['DIRO']}circ_mask/torrential.contribution.{env_vars['FILE_NAME']}.w.t.{env_vars['THRESHOLD']:.2f}.{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}.circ.mask.{env_vars['RADIUS']:.1f}km.nc"))
        name = f"{script_filename[:-3]}__{env_vars['FILE_NAME']}_{env_vars['THRESHOLD']:.2f}_{env_vars['YEAR']:04d}{env_vars['MONTH']:02d}_circ_mask_{env_vars['RADIUS']:.1f}km"
    ...
    if len(jobs_id) != 0:
        submit_job.check.check_job_status(jobs_id, time_delay=300)
