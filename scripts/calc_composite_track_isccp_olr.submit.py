...
for level in levels:
    ...
    for mode in modes:
        ...
        if env_vars["FILTERED"] == 0:
            files = sorted(glob(f"{env_vars['DIRO']}composite.anom.{env_vars['FILE_NAME']}.{mode_dict[mode]}.{env_vars['YEAR_START']}-{env_vars['YEAR_END']}.nc"))
            name = f"{script_filename[:-3]}__anom_{env_vars['FILE_NAME']}_{mode_dict[mode]}_{env_vars['YEAR_START']}-{env_vars['YEAR_END']}"
