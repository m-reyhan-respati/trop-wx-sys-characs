...
script_filename = "calc_mean_extreme_contribution_gpm_track_isccp_olr_circ_mask.py"
...
env_vars["DIRI"] = f"{SCRATCH_GPM_DIR}/3hr_{env_vars['FILE_NAME']}/extreme_contribution/circ_mask/"
env_vars["DIRO"] = f"{SCRATCH_GPM_DIR}/3hr_{env_vars['FILE_NAME']}/mean_extreme_contribution/circ_mask/"
...
for mode in modes[0:3]:
    ...
    for i in range(0, len(lat_min_list)):
        ...
        files = sorted(glob(f"{env_vars['DIRO']}mean.extreme.contribution.{env_vars['PERCENTILE']:d}p.{env_vars['FILE_NAME']}.{mode_dict[mode]}.circ.mask.{env_vars['RADIUS']:.1f}km.{lat_min_string}-{lat_max_string}.nc"))
        name = f"{script_filename[:-3]}__{env_vars['PERCENTILE']:d}p_{env_vars['FILE_NAME']}_{mode_dict[mode]}_circ_mask_{env_vars['RADIUS']:.1f}km_{lat_min_string}-{lat_max_string}"
