...
script_filename = "calc_count_raining_track_isccp_olr.py"

env_vars = {}
env_vars["DIRI"] = f"{SCRATCH_TRACK_DIR}/object_mean_precipitation/"
...
for mode in modes:
    ...
    files = sorted(glob(f"{env_vars['DIRO']}count*raining.track.txuptp.{mode_dict[mode]}.nc"))

    if len(files) != 0:
        continue

    name = f"{script_filename[:-3]}__{mode_dict[mode]}"
